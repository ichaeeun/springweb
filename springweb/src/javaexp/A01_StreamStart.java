...
/*
 ...
 1. Stream 
  ...
  6. FileInputSream 및 FileOutputSystem
  	시스템에 있는 모든 파일을 읽거나 쓸 수 있는 기능을 제공 
  	생성자로 스트림객체를 생성할 때는 FileNotFoundException 예외 가능성이 
  	있기 때문에 반드시 에외 처리 필요 
  ...
  8. DataInputStream 및 DataOutputStream 
  	1) 각각 기초 타입 데이터를 읽는 메서드와 기초 타입 데이터를 기록하는 메서드를 
  		사용할 수 있는 스트림 
  	2) 직접 키보드에서 데이터를 입력 받거나 콘솔 뷰에 데이터를 출력하기에는 부적합 
  	FileInputSteam 및 FileOutputStream 등 다른 스트림과 연결해서 
  	파이프라인을 구성해 사용 
  9. 문자스트림 
  	데이터를 2바이트 단위인 유니코드로 전송하거나 수신하는데, 이진 데이터로 된 이미지나 
  	동영상 파일 보다는 한글처럼 언어로 된 파일을 처리할 때 유용 
  	Reader 와 Writer는 객체를 생성할 수 없는 추상 클래스 이기 때문에 
  	Reader 와 Writer의 자식인 구현 클래스를 사용 
  	FileReader와 FileWriter는 파일 입출력 클래스로, 파일에서 문자 데이터를 
  	읽거나 파일에 문자 데이터를 저장할 때 사용 
  	InputStreamReader 및 OutputStreamReader는 바이트 스트림과 문자스트림을 연결하는 
  	브리지 스트림으로 사용 
  	BufferedReader와 BufferWriter는 데이터를 효율저그올 전송하려고 
  	버퍼로 처리할 때 사용 
  10. Reader와 Writer
 ...
 */
		...
		// 키보드를 통해서 자바에서 기본 입력장치(console)에 입력 처리 
		is = System.in;
		try {
			int n = is.read();
			// 입력받은 내용을 int로 가져온다. io발생 
			// io발생 시 반드시 예외처리 해준다. 
			// 읽은 내용을 문자로 변환 출력 
			System.out.println("입력된값: "+(char)n);
		} catch (IOException e) {
