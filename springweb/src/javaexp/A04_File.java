...
/*
 ...
 12. 파일 관리 
 	...
 	입출력 스트림에 파일 생성 혹은 삭제, 파일 속성 변경등의 관리 기능은 없음 
 	자바는 파일을 관리하기 위해 File클래스를 제공 
 	여전히 File 클래스를 많이 사용하지만, 자바 4부터 도입되고 자바7에서 기능을 
 	보완한 NIO(java.nio) 및 NIO2(java.nio2) 기반의 
 	Path 인터페이스, Files클래스 및 FileChannel도 유용 
 13. File 클래스 
 ...
 15. 채널 
 	...
 	IO기반의 파일 처리보다 복잡하고 성능이 떨어질 수 있다. 
 	
 	/Users/ichaeeun/git/springweb/springweb/src/javaexp/z01_test.txt
 */
		try {
			String path="/Users/ichaeeun/git/springweb/springweb/src/javaexp";
		...
		//	f.createNewFile();
			...
			boolean b2 = new File(path,"z02_test.txt").delete(); //파일 삭제처리 
			String [] listing = new File(path).list();
			// 해당 경로에 있는 파일명 리스트 
			for(String str:listing) {
				System.out.println("현재폴더내용: "+str);
			}
		}catch(Exception e) {
			e.printStackTrace();
		}
	/*
	 ex) z01_test/z02_test.txt 임의의 데이터로 파일저장하고 
	 	해당 파일 정보를 읽어오고, 해당 파일의 리스트를 읽어 오고, 
	 	마지막으로 삭제처리 하세요 
	 */
		try {
			String path = "/Users/ichaeeun/git/springweb/springweb/src/javaexp/z01_test";
			File f = new File(path,"z02_test.txt");
			System.out.println(f.getName());
			System.out.println(f.exists());
			System.out.println(f.getPath());
			System.out.println(f.length());
			System.out.println(File.separator);
			boolean b = new File(path+"/z02_test").mkdir();
			String [] list = new File(path).list();
			System.out.println("현재 폴더 내용");
			for(String str:list) {
				System.out.println(str);
			}
			boolean b2 = new File(path,"z03_test.txt").delete();
		}catch(Exception e){
			e.printStackTrace();
		}
