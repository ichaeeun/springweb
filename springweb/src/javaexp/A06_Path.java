...
/*
 ...
 2. 주요 기능 메서드 
 	...
 	3) getRoot() : root경로 정보 
 	4) isAbsolute() : 절대경로 인지 여부 
 	5) toUri() : 브라우저에서 열 수 있는 Path 반환 처리 
 	6) resolve() : 경로를 결합하여 새로운 경로를 가져온다. 
 	
 # Paths 
 1. 여러개의 파일이나 폴더를 관리하는 객체 
 2. 주요 기능 메서드 
 ...
 */
		...
		// toUri() : 브라우저에서 열수 있는 path 반환 
		URI ul = path.toUri();
		System.out.println(ul);
		// toAbsolutePath() : 절대 경로를 사용하는 path 반환 
		Path xxx = Paths.get(ul);
