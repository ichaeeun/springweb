...
/*
 ...
 3. 키는 중복되지 않고, 하나의 값에만 매핑되어  있으므로 
 	키가 있다면 대응하는 값을 얻을 수 있다. 
 4. 주요 메서드 
 	1) isEmpty() : 컬렉션이 비어 있는지 여부를 반환함 
 	2) get(key) : 특정한 key에 해당하는 값을 반환 
 	3) Put(key,value) : 해당 객체에 key,value를 입력 처리 한다. 
 ...
 */		
		...
		gMap.put(1235, 50);
		gMap.put(1236, 60);
		Set<Integer> keys2 = gMap.keySet();
		for(int k:keys2) {
			System.out.println("번호: "+k+"점수: "+gMap.get(k));
		}
